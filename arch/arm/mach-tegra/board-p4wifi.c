...
static void p3_s5k5ccgx_power_off(void)
{
	...
	if (system_rev < 14)
		...
		msleep(200);
		printk(KERN_DEBUG "p3_s5k5ccgx_power_off\
			msleep--------200ms  system_rev = %d\n", system_rev);
	}
}
...
static int P3_s5k5ccgx_flash(int enable)
{
	...
	printk(KERN_DEBUG "========== flash enable = %d\n", enable);
...
static int P3_s5k5ccgx_af_assist(int enable)
{
	...
	printk(KERN_DEBUG "========== flash af_assist =========== %d\n",\
			enable);
#if 0
	gpio_set_value(GPIO_CAM_FLASH_EN, enable);
	if (enable) {
